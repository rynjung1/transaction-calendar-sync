...
- **Session storage is currently plain `AsyncStorage`** (`mobile/src/lib/supabase.ts`) — unencrypted on-device storage on both platforms. For a financial app this should be `expo-secure-store` (iOS Keychain / Android Keystore-backed) instead. In progress — see status note below.

### Mobile security (findings from a dedicated review, not yet all fixed)
- **App Store blocker, verified by reading the actual native source, not assumed**: `expo-dev-client`'s launcher screen ships in *every* EAS build profile, including `production` — the `developmentClient` flag in `eas.json` has no effect on this at all. Traced through `expo-dev-launcher`'s `EXDevLauncherController.m`: when a local `main.jsbundle` is embedded, it unconditionally calls `navigateToLauncher()` (a comment there claiming a separate "autoload" path exists in `DevLauncherViewController.swift` is stale — that code doesn't exist). There is no build-profile or Debug/Release conditional anywhere in the plugin (`expo-dev-client/plugin/src/withDevClient.ts`) or the native controller. Concretely: a real user downloading this from the App Store today would see "Development Build" / "Searching for development servers..." instead of the sign-in screen. Must be resolved (switch to a dynamic `app.config.js` that conditionally excludes `expo-dev-client` for the real store build, or a deliberate dev-tooling-stripping pass before submission) before any App Store submission — this is a real architecture decision, not a one-line fix, and hasn't been picked yet.
- **Calendar privacy, a design implication rather than a bug**: `createTransactionEvent()` (`mobile/src/lib/calendar.ts`) puts the merchant name and exact dollar amount directly into the calendar **event title** — visible on lock-screen previews/notifications, and inherits whatever sharing settings the chosen calendar has. `CalendarPickerScreen.tsx` doesn't warn the user about this when they pick a shared calendar. Not fixed; flagging as a real, not hypothetical, exposure surface for anyone who links their bank to a shared family calendar.
- Passed cleanly, checked directly rather than assumed: no `NSAppTransportSecurity`/`NSAllowsArbitraryLoads` exceptions weakening TLS in `app.json`; no custom `Linking`/deep-link handling in the app's own code that could act on attacker-controlled URL parameters (Supabase client has `detectSessionInUrl: false`).
