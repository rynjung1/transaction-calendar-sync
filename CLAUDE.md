...
- **Still genuinely unresolved — a fix attempt here did not fix it, verified**: a transaction with no Plaid PFC enrichment can never be excluded by category, because `sync_filters.excluded_categories` only ever holds PFC `primary` values and such a transaction has none to compare. Tried isolating the comparison to PFC-only (`shouldSyncTransaction()` in `backend/lib/plaidSync.ts`, now a pure exported function) expecting that to close the gap — it doesn't: a real before/after test proved the old inline logic and the new PFC-only version produce the *identical* result for this case (both leave the transaction synced). The refactor is worth keeping (directly unit-testable now), but the underlying limitation is unchanged. A real fix needs a legacy-to-PFC taxonomy mapping, which risks introducing wrong exclusions if done inaccurately — not attempted. Stops being purely theoretical the moment someone actually uses the settings screen (`mobile/src/screens/SettingsScreen.tsx`, built but not yet runtime-verified) to exclude a category expecting it to apply universally.
