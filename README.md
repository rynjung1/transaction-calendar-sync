...
- `0006_nullable_revoked_access_token.sql` — drops the `NOT NULL` constraint on `plaid_items.access_token_encrypted`, letting the webhook handler null out a token Plaid has already confirmed dead (`USER_PERMISSION_REVOKED`/`USER_ACCOUNT_REVOKED`). **Not yet applied** — same manual step, and `webhook.ts` runs correctly either way (falls back to a status-only update, verified live).
...
4. A few housekeeping items: rotating `PLAID_SECRET` (precautionary, see CLAUDE.md), running the pending `0004`/`0005`/`0006` migrations, sharing the Privacy Policy / Terms of Service artifacts publicly (both currently private-by-default), and a real app icon pass (current one is a deliberate but non-final Pillow placeholder).
