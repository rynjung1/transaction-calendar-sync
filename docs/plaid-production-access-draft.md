...
**Flag this before submitting**: as documented in `CLAUDE.md`, the code-side scaffolding for this is now built and live-verified (iOS Associated Domains, the `apple-app-site-association` file, the redirect landing page, conditional `redirect_uri` wiring) — but it's genuinely inert until three account actions happen: a real Apple Developer Team ID replaces the AASA file's current placeholder, the actual redirect URI (`https://backend-theta-fawn-72.vercel.app/plaid-oauth-return`) gets registered in the Plaid Dashboard's Allowed redirect URIs list, and `PLAID_REDIRECT_URI` gets set as a Vercel env var. Several major Canadian banks (RBC, TD, Scotiabank, BMO, CIBC — all visible in this app's own institution search) commonly require OAuth-based Plaid Link, so this needs to be finished — and tested on a real device through an actual OAuth-requiring bank — before real users with one of those banks can complete Link in production, not just before the application itself.
